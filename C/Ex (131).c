/*O programa abaixo realiza operações em uma lista de caracteres duplamente 
encadeada (lista onde cada nó faz referência ao próximo nó e também ao 
anterior). Cada operação é realizada de acordo com a escolha do usuário. 
As operações são: inserir nó no início, inserir no final, remover nó, 
imprimir na ordem direta de entrada e imprimir na ordem inversa de entrada.*/
